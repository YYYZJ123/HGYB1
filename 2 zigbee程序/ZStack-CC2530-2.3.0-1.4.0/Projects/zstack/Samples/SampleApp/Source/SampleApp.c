...
void SampleApp_MessageMSGCB( afIncomingMSGPacket_t *pkt )
{
  ...
  {
 #ifdef ZDO_COORDINATOR
    case SAMPLEAPP_P2P_CLUSTERID:  //接收数据
      {
        id=pkt->cmd.Data[0];
        wendu=pkt->cmd.Data[1];
        shidu=pkt->cmd.Data[2];
        s[id]=id+0x30;   //储存设备连接情况
        if(pkt->cmd.Data[3]!=0)
          sprintf(strTemp, "x%da%db%d\r\n",id,wendu,shidu);
        else
          sprintf(strTemp, "y%da%db%d\r\n",id,wendu,shidu);
        HalUARTWrite(0, strTemp, osal_strlen(strTemp)); //输出接收到的数据
      }
      break; 
#endif
...
void SampleApp_Send_P2P_Message( void )
{
  ...
  //DHT11()获取温湿度并分别赋值十位和个位
  //TOF用于判断传感器连接情况，读取数据成功赋1，失败赋0。
  TOF=DHT11(); 
  Delay_ms(500);
  //将温湿度的转换成字符串,供LCD显示  
